...
export default function MyShipmentsPageWrapper() {
  return (
...
function MyShipmentsPage() {
  ...
  useEffect(() => {
    ...
    const newShipment = searchParams.get('new');
  ...
  const handleOpenChange = (isOpen: boolean) => {
    ...
    if (!isOpen) {
        resetForm();
        router.push('/dashboard/exporter/my-shipments', { scroll: false });
    }
  }
  ...
  const handleValidation = () => {
    ...
    if (parseFloat(numberOfPackages) <= 0) {
        toast({ title: "Invalid Input", description: "Number of packages must be a positive number.", variant: "destructive" });
        return false;
    }
    if (parseFloat(weight) <= 0) {
        toast({ title: "Invalid Input", description: "Total Weight must be a positive number.", variant: "destructive" });
        return false;
    }
    if (!user) {
